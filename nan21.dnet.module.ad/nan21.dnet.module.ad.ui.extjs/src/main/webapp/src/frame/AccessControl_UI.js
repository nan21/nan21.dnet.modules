...
Ext.define("net.nan21.dnet.module.ad.usr.frame.AccessControl_UI", {  
	...
	,_defineElements_: function() {							
		...
		.addDcView("ctrl",{ name:"privilegeEditList", xtype:"net.nan21.dnet.module.ad.usr.dc.AccessControl$EditList", frame:true,dockedItems:[{ xtype:"toolbar", ui:"footer", dock: 'bottom', weight:-1, items:[ this._elems_.get("btnAsgnRoleToAccessCtrl") ,this._elems_.get("btnCopyRules") ,this._elems_.get("btnShowUiAsgnRules") ,this._elems_.get("btnShowUiDsRules") ]}]})	 
...
;	 	
	...
	,_defineToolbars_: function() {
		this._getBuilder_()
			...
			.beginToolbar("tlbDsAccessCtxEditList", {dc:"dsAccess"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addSeparator().addAutoLoad().addSeparator().addSeparator().addTitle({"text":"Data-source"}).end()
			.beginToolbar("tlbAsgnAccessCtxEditList", {dc:"asgnAccess"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addSeparator().addAutoLoad().addSeparator().addSeparator().addTitle({"text":"Assignment"}).end(); 	
	}

...
Ext.define("net.nan21.dnet.module.sd.opportunity.dc.Opportunity$Filter", {
	...
	,_linkElements_: function () {
		...
		.addChildrenTo("col1",["name","account"])
...
Ext.define("net.nan21.dnet.module.sd.opportunity.dc.Opportunity$List", {
	...
	,_defineColumns_: function () {	
		...
		.addDateColumn({ name:"expectedCloseDate", dataIndex:"expectedCloseDate",format:Ext.DATE_FORMAT})   	      	     
		.addNumberColumn({ name:"expectedAmount", dataIndex:"expectedAmount",format:Ext.NUMBER_FORMAT_DEC })  
		.addTextColumn({ name:"expectedCurrency", dataIndex:"expectedCurrency",width:100 })   	
		.addNumberColumn({ name:"probability", dataIndex:"probability",format:Ext.NUMBER_FORMAT_DEC })  
...
Ext.define("net.nan21.dnet.module.sd.opportunity.dc.Opportunity$Edit", {
	...
	_defineElements_: function () {	
		...
		.addDateField({ name:"expectedCloseDate", dataIndex:"expectedCloseDate",anchor:"-20" ,allowBlank:false})
		.addNumberField({ name:"probability", dataIndex:"probability",anchor:"-20"  , style: "text-align:right;" })
		.addNumberField({ name:"expectedAmount", dataIndex:"expectedAmount",anchor:"-20" ,allowBlank:false , style: "text-align:right;" })
		.addLov({ name:"expectedCurrency", xtype:"net.nan21.dnet.module.bd.currency.lovs.Currencies", dataIndex:"expectedCurrency",anchor:"-20" ,maxLength:32,retFieldMapping: [{lovField:"id", dsField: "expectedCurrencyId"} ]  })
		.addTextField({ name:"assignedTo", dataIndex:"assignedTo",anchor:"-20" ,maxLength:255  })
		//containers
...
labelAlign:"top"}})     
	...
	,_linkElements_: function () {
		this._getBuilder_()
		.addChildrenTo("main",["col1" ,"col2" ,"col3" ])
		.addChildrenTo("col1",["name","account","salesStage","expectedAmount","expectedCurrency","expectedCloseDate"])
		.addChildrenTo("col2",["status","priority","leadSource","campaign","probability","resultReason"])

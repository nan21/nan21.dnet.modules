...
Ext.define("net.nan21.dnet.module.sd.order.frame.SalesOrderBaseData_UI", {  
	...
	,_defineElements_: function() {							
		...
		.addPanel({name: "canvasOrderType", layout:"border", defaults:{split:true},title:"Sales order type",preventHeader:true})  	 
		.addPanel({name: "canvasOrderStatus", layout:"border", defaults:{split:true},title:"Sales order status",preventHeader:true})  	 
	...
	,_defineToolbars_: function() {
		this._getBuilder_()
			.beginToolbar("tlbOrderType", {dc:"orderType"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addReports().addSeparator().addSeparator().addTitle({"text":"Sales order type"}).end()
			.beginToolbar("tlbOrderStatus", {dc:"orderStatus"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addReports().addSeparator().addSeparator().addTitle({"text":"Sales order status"}).end(); 	
	}

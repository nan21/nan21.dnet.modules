...
Ext.define("net.nan21.dnet.module.pj.md.frame.Projects_UI", {  
	...
	,_defineElements_: function() {							
		...
		.addDcEditGridView("version",{ name:"versionEditList", xtype:"net.nan21.dnet.module.pj.md.dc.ProjectVersion$CtxEditList", frame:true,title:"Versions",dockedItems:[{ xtype:"toolbar", ui:"footer", dock: 'bottom', weight:-1, items:[ this._elems_.get("btnRoadmapUi") ,this._elems_.get("btnChangelogUi") ]}]})	 
		.addDcFilterFormView("component",{ name:"componentFilter", xtype:"net.nan21.dnet.module.pj.md.dc.ProjectComponent$Filter"})	 
		.addDcEditGridView("component",{ name:"componentEditList", xtype:"net.nan21.dnet.module.pj.md.dc.ProjectComponent$CtxEditList", frame:true,title:"Components"})	 
		...
		.addDcEditGridView("member",{ name:"memberEditList", xtype:"net.nan21.dnet.module.pj.md.dc.ProjectMember$CtxEditList", frame:true,title:"Team"})	 
		.addPanel({name: "main",layout:"card", activeItem:0})  	 

		.addPanel({name: "projectDetailsTab", xtype:"tabpanel", activeTab:0, plain:false, deferredRender:false, id:Ext.id()}) 	 
...
;	 	
	...
	,_defineToolbars_: function() {
		this._getBuilder_()
			.beginToolbar("tlbProjectList", {dc:"project"}).addQuery().addEdit().addNew().addCopy().addDeleteSelected().addReports().addSeparator().addSeparator().addTitle({"text":"Projects"}).end()
			.beginToolbar("tlbProjectEdit", {dc:"project"}).addBack().addSave().addNew().addCopy().addCancel().addPrevRec().addNextRec().addReports().addSeparator().addSeparator().addTitle({"text":"Project"}).end()
			.beginToolbar("tlbVersionEditList", {dc:"version"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addSeparator().addAutoLoad().addReports().addSeparator().addSeparator().addTitle({"text":"Versions"}).end()
			.beginToolbar("tlbComponentEditList", {dc:"component"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addSeparator().addAutoLoad().addReports().addSeparator().addSeparator().addTitle({"text":"Components"}).end()
			.beginToolbar("tlbMemberEditList", {dc:"member"}).addQuery().addSave().addNew().addCopy().addDeleteSelected().addCancel().addSeparator().addAutoLoad().addReports().addSeparator().addSeparator().addTitle({"text":"Team"}).end()

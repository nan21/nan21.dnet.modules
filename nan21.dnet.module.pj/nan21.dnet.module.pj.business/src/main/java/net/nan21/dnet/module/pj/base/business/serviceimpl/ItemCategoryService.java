/*    
 * DNet eBusiness Suite
 * Copyright: 2008-2011 Nan21 Electronics SRL. All rights reserved.
 * Use is subject to license terms. 
 */
package net.nan21.dnet.module.pj.base.business.serviceimpl;

import net.nan21.dnet.core.business.service.AbstractEntityService;
import net.nan21.dnet.module.pj.base.business.service.IItemCategoryService;

import javax.persistence.EntityManager;
import net.nan21.dnet.module.pj.base.domain.entity.ItemCategory;

public class ItemCategoryService extends AbstractEntityService<ItemCategory>
        implements IItemCategoryService {

    public ItemCategoryService() {
        super();
    }

    public ItemCategoryService(EntityManager em) {
        super();
        this.em = em;
    }

    @Override
    protected Class<ItemCategory> getEntityClass() {
        return ItemCategory.class;
    }

    public ItemCategory findByName(Long clientId, String name) {
        return (ItemCategory) this.em
                .createNamedQuery(ItemCategory.NQ_FIND_BY_NAME)
                .setParameter("pClientId", clientId)
                .setParameter("pName", name).getSingleResult();
    }

}

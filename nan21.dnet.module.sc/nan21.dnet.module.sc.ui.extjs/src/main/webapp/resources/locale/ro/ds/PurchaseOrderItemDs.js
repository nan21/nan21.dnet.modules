
Ext.define("net.nan21.dnet.module.sc.order.ds.model.PurchaseOrderItemDs$Trl", {  	  	
	uuid__lbl :  "@uuid",	entityFQN__lbl :  "@entityFQN",	purchaseOrderId__lbl :    "Comandă furnizor(ID)",
	productId__lbl :    "Articol(ID)",
	productCode__lbl :    "Articol",
	productName__lbl :    "Articol",
	uomId__lbl :    "UM(ID)",
	uomCode__lbl :    "UM",
	quantity__lbl :  "Cantitate",
	unitPrice__lbl :  "Preţ unitar",
	netAmount__lbl :  "Valoare",
	taxAmount__lbl :  "Valoare taxe",
	taxId__lbl :    "Taxă(ID)",
	tax__lbl :    "Taxă",
		 
	id__lbl :"ID"
});
